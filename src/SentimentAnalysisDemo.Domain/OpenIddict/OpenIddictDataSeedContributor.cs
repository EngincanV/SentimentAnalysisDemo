...
namespace SentimentAnalysisDemo.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("SentimentAnalysisDemo") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "SentimentAnalysisDemo", DisplayName = "SentimentAnalysisDemo API", Resources = { "SentimentAnalysisDemo" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["SentimentAnalysisDemo_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["SentimentAnalysisDemo_Web:RootUrl"]!.EnsureEndsWith('/');

            /* SentimentAnalysisDemo_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["SentimentAnalysisDemo_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["SentimentAnalysisDemo_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["SentimentAnalysisDemo_Swagger:RootUrl"]?.TrimEnd('/');
